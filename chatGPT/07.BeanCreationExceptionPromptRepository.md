...
ChatGPT:
You're very close, but there are a couple of issues.

### 1. `redirectAttributes` won't work here
...
which is rendering a view directly, **not redirecting**.

So you don't need `RedirectAttributes` at all. Just add the warning to the `Model`:
...
### 2. Only add the warning if a keyword was found

Right now your code always adds the warning, even when `flagged == false`.
...
but you never save the updated prompt.

If you want the flag to persist in the database:
...
### 4. Store the keyword that triggered the flag

The assignment says admins should see the flagged keyword.
...
### 5. Display the warning in `ai-response.html`
...
Place that near the top of the page.
...
That satisfies the requirement nicely because the warning appears when the user submits to AI, and the prompt is stored as flagged for later admin review.
